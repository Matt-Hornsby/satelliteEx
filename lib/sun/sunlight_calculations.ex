defmodule Sun.SunlightCalculations do
  ...
  def calculate_magnitude(
    ...
    base_magnitude =
      get_base_magnitude(satellite_magnitude, satellite_position, sun_position, observer, gmst)
  ...
  defp calculate_sunlit_status(satellite_position, sun_position) do
    k =
      satellite_position.x * sun_position.x_plane +
  ...
  defp sunlit?(k, satellite_position) do
    new_k =
      :math.sqrt(
        satellite_position.x * satellite_position.x +
  ...
  defp get_base_magnitude(
    ...
    observer_geoc_x =
      :math.cos(observer_geocentric.geo_lat) * :math.cos(observer_position.longitude_rad)

    observer_geoc_y =
      :math.cos(observer_geocentric.geo_lat) * :math.sin(observer_position.longitude_rad)

    observer_geoc_z = :math.sin(observer_geocentric.geo_lat)

    qx =
      satellite_position.x -
        observer_geoc_x * Constants.earth_radius_semimajor() * :math.cos(-sidereal_angle) -
        observer_geoc_y * Constants.earth_radius_semimajor() * :math.sin(-sidereal_angle)

    qy =
      satellite_position.y +
        observer_geoc_x * Constants.earth_radius_semimajor() * :math.sin(-sidereal_angle) -
        observer_geoc_y * Constants.earth_radius_semimajor() * :math.cos(-sidereal_angle)
    ...
    xmag =
      -15.75 + 2.5 * :math.log(range * range / phasefactor) / :math.log(10) + standard_magnitude

    xmag
  end

  defp adjust_magnutide_for_low_elevation(base_magnitude, elevation) when elevation < 20 do
    # credo:disable-for-next-line
    base_magnitude + (20 - elevation) / 15 * 1
  end

  defp adjust_magnutide_for_low_elevation(base_magnitude, _elevation), do: base_magnitude

  defp adjust_magnitude_for_sunset(base_magnitude, sun_elevation)
       when sun_elevation > -10 and sun_elevation < 0 do
    base_magnitude + (10 + sun_elevation) / 10 * 6
  end

  defp adjust_magnitude_for_sunset(base_magnitude, _sun_elevation), do: base_magnitude
end
