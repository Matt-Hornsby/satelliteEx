...
defmodule Satellite.SGP4.Init do
  ...
  def init(satrec, init_parameters) do
    ...
    if omeosq >= 0.0 || satrec.no >= 0.0 do
      isimp = if rp < 220.0 / Constants.earth_radius_semimajor() + 1.0, do: 1, else: 0
      ...
      satrec = %{
        satrec
        | mdot:
            satrec.no + 0.5 * temp1 * rteosq * satrec.con41 +
              0.0625 *
      ...
      # so I am blowing it off for now.
      # (it's hundreds of lines of code that I don't have a good way of testing
      # at the moment)
      ...
      satrec =
        if satrec.isimp === 1 do
          ...
          new_satrec = %{
            new_satrec
            | d4: 0.5 * temp * ao * tsi * (221.0 * ao + 31.0 * sfour) * new_satrec.cc1
          }
          ...
          new_satrec = %{
            ...
            | t4cof:
                0.25 *
                  (3.0 * new_satrec.d3 + new_satrec.cc1 * (12.0 * new_satrec.d2 + 10.0 * cc1sq))
          }
