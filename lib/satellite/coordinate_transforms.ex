defmodule Satellite.CoordinateTransforms do
  ...
  def ecf_to_look_angles(
        %Observer{latitude_rad: latitude, longitude_rad: longitude} = observer_coordinates_ecf,
        satelliteCoordsEcf
      ) do
    observer_ecf = geodetic_to_ecf(observer_coordinates_ecf)
    ...
    top_s =
      :math.sin(latitude) * :math.cos(longitude) * rx +
        :math.sin(latitude) * :math.sin(longitude) * ry - :math.cos(latitude) * rz

    top_e = -:math.sin(longitude) * rx + :math.cos(longitude) * ry

    top_z =
      :math.cos(latitude) * :math.cos(longitude) * rx +
        :math.cos(latitude) * :math.sin(longitude) * ry + :math.sin(latitude) * rz
  ...
  defp topocentric_to_look_angles(topocentricCoords) do
    ...
    %{
      azimuth_rad: az,
      azimuth_deg: az * Constants.rad2deg(),
      elevation_rad: el,
      range_sat: range_sat,
      elevation_deg: el * Constants.rad2deg()
    }
  end
  ...
  def eci_to_geodetic(eci_coords, gmst) do
    a = Constants.earth_radius_semimajor()
    b = Constants.earth_radius_semiminor()
  ...
  def iterate_latitude(iteration_params, k, kmax) when k < kmax do
    c =
      1 /
        :math.sqrt(
          1 -
            iteration_params.e2 *
              (:math.sin(iteration_params.latitude) * :math.sin(iteration_params.latitude))
        )
    ...
    iteration_params = %{
      ...
      | c: c,
        latitude:
          :math.atan2(
            iteration_params.eci_z +
              iteration_params.a * c * iteration_params.e2 * :math.sin(iteration_params.latitude),
            iteration_params.r
          )
    }
  ...
  def iteration_params(iteration_params, _k, _kmax),
    do: %{a: iteration_params.c, latitude: iteration_params.latitude}
end
